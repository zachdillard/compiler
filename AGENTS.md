...
- Use `dotnet run --project Compiler -- --lex <source-file>`, `--parse`, or `--codegen` to stop after an intermediate stage. These stages must not leave an assembly file or an executable behind; the book's test runner checks for them.
